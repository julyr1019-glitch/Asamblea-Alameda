...
# --- 3. CSS DIFERENCIADO ---
rol = st.sidebar.radio("SISTEMA", ["Votante", "Administrador"], key="nav_acta")
...
if rol == "Administrador":
    ...
    clave = st.text_input("Contraseña Maestro:", type="password", key="pwd_acta")
    ...
    if clave == "Alameda2026*":
        ...
        if st.sidebar.button("🔄 ACTUALIZAR (RERUN)", use_container_width=True):
            st.rerun()
        if st.sidebar.button("🧹 REINICIAR DATA", type="secondary", use_container_width=True):
        ...
        cm2.metric("Casas", f"{len(servidor['conectados'])}")
        ...
        if not servidor["asamblea_iniciada"]:
            ...
            sel_p = st.selectbox("Pregunta actual:", range(len(preguntas)), index=servidor['p_idx'], format_func=lambda x: preguntas[x])
            ...
            # --- VISUALIZACIÓN DE RESULTADOS (PARA CAPTURA) ---
            df_v = servidor['votos']
            ...
            if not v_act.empty:
                ...
                # Gráfica más pequeña para captura
                ...
                # --- TABLA DE RESULTADOS EN VENTANA ---
                st.markdown("### 📋 Detalle de Votos por Casa")
                st.dataframe(v_act[['casa', 'representa', 'voto']].sort_values(by='casa'), 
                             use_container_width=True, hide_index=True)
            
            # --- DESCARGA DE EXCEL MEJORADA ---
            if not df_v.empty:
                df_export = df_v.copy()
                # Cambiar p_id por el texto de la Pregunta
                df_export['Pregunta'] = df_export['p_id'].apply(lambda x: preguntas[x])
                # Reordenar y limpiar
                df_export = df_export[['casa', 'representa', 'Pregunta', 'voto']]
                
                csv = df_export.to_csv(index=False).encode('utf-8')
                st.download_button("📥 Descargar Reporte para el Acta", data=csv, 
                                   file_name=f"Resultados_Alameda_{datetime.now().strftime('%d_%m')}.csv",
...
# --- VISTA VOTANTE ---
    ...
    if 'mi_casa' not in st.session_state:
        ...
        st.sidebar.info(f"📍 Casa: {casa} | 🗳️ Votos: {repre}")
        
        if not servidor["asamblea_iniciada"]:
            st.warning("⏳ Plataforma en preparación..."); time.sleep(3); st.rerun()
        
        fase, p_id = servidor['fase'], servidor['p_idx']
        
        if fase == "espera":
            st.info("⌛ Esperando pregunta..."); time.sleep(3); st.rerun()
        else:
